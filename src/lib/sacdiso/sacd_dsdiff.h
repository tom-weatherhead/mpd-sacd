...
public:
	sacd_dsdiff_t();
	virtual ~sacd_dsdiff_t() override;
	bool open(sacd_media_t* sacd_media, open_mode_e mode = MODE_MULTI_TRACK) override;
	bool close() override;
	uint32_t get_tracks() override;
	uint32_t get_tracks(area_id_e area_id = AREA_BOTH) override;
	uint32_t get_channels() override;
	uint32_t get_loudspeaker_config() override;
	uint32_t get_samplerate() override;
	uint16_t get_framerate() override;
	...
	void get_info(uint32_t track_index, TagHandler& handler) override;
	bool is_dst() override;
	void set_emaster(bool emaster) override;
	void select_area(area_id_e area_id) override;
	bool select_track(uint32_t track_index, area_id_e area_id = AREA_BOTH, uint32_t offset = 0) override;
